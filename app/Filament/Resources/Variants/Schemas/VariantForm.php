...
class VariantForm
{
        ...
        return $schema
            ->components([
                ...
                Repeater::make('variant_features')
                    ...
                    ->hint('Add combinations of features and values that define this variant. Features are based on the selected product.')
                    ->schema([
                        Select::make('feature_id')
                            ...
                            ->preload()
                            ->live(onBlur: true)
                            ->afterStateUpdated(fn ($state, callable $set) => $set('feature_value', null))
                    ...
                    ->itemLabel(function (array $state, callable $get): ?string {
                        $productId = $get('../../product_id');
                        if (!$productId || !$state['feature_id'] || !$state['feature_value']) {
                            return 'New Feature';
                        }
                        ...
                        $valueName = $valuesMap[$state['feature_value']] ?? 'Value';
    ...
    /**
     * Get features map for a specific product (id => name)
     */
    ...
    /**
     * Get feature values for a specific product and feature
     */
    protected static function getProductFeatureValues(int $productId, int $featureId): array
    {
        ...
        return collect($feature['values'])
            ->mapWithKeys(fn ($value) => [
                $value['value'] => $value['value'] ?? $value['name'] ?? ('Value ' . $value['id']),
            ])->toArray();
    }
